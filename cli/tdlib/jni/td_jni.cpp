...
static jlong TelegramRawClient_create(JNIEnv *env, jclass clazz) {
  return static_cast<jlong>(reinterpret_cast<std::uintptr_t>(new td::Client()));
}

static void TelegramRawClient_send(JNIEnv *env, jclass clazz, jlong client_id, jlong id, jobject function) {
  get_client(client_id)->send({static_cast<std::uint64_t>(id), fetch_function(env, function)});
}

static jint TelegramRawClient_receive(JNIEnv *env, jclass clazz, jlong client_id, jlongArray ids, jobjectArray events,
                                       jdouble timeout) {
...
static jobject TelegramRawClient_execute(JNIEnv *env, jclass clazz, jobject function) {
  jobject result;
  td::Client::execute({0, fetch_function(env, function)}).object->store(env, result);
  return result;
}

static void TelegramRawClient_destroy(JNIEnv *env, jclass clazz, jlong client_id) {
...
static jint register_native(JavaVM *vm) {
  ...
  auto client_class = td::jni::get_jclass(env, PACKAGE_NAME "/TelegramRawClient");
  log_class = td::jni::get_jclass(env, PACKAGE_NAME "/TelegramLog");
...
#define TD_FUNCTION "L" API_PACKAGE_NAME "/TdApi$Function;"
  ...
  register_method(client_class, "create", "()J", TelegramRawClient_create);
  register_method(client_class, "send", "(JJ" TD_FUNCTION ")V", TelegramRawClient_send);
  register_method(client_class, "receive", "(J[J[" TD_OBJECT "D)I", TelegramRawClient_receive);
  register_method(client_class, "execute", "(" TD_FUNCTION ")" TD_OBJECT, TelegramRawClient_execute);
  register_method(client_class, "destroy", "(J)V", TelegramRawClient_destroy);
